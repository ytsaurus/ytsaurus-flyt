...
def _normalize_nt_shlex_tokens(tokens: List[str]) -> List[str]:
    """Windows ``shlex.split(posix=False)`` often leaves matching outer quotes inside the token; strip one pair."""
    out: List[str] = []
    for t in tokens:
...
def split_job_command_tokens(job_command: str) -> List[str]:
    """Split job command like a shell (POSIX on Unix; Windows-safe for paths and quoted args)."""
    s = (job_command or "").strip()
    if not s:
        return []
    parts = shlex.split(s, posix=os.name != "nt")
    if os.name == "nt":
        return _normalize_nt_shlex_tokens(parts)
    return parts


def first_script_path_from_job_command(job_command: str) -> Optional[str]:
    """First token that looks like a path to a .py file."""
    parts = split_job_command_tokens(job_command)
    if not parts:
        return None
    first = parts[0]
